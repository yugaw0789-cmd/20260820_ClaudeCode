...
allowed-tools: ["Read", "Glob", "Grep", "Bash", "Write", "Edit", "AskUserQuestion", "Workflow", "Workflow(claude-security:scan)", "TaskCreate", "TaskGet", "TaskList", "TaskUpdate", "TaskOutput", "TaskStop", "Agent(claude-security:scan-inventory", "claude-security:scan-researcher", "claude-security:scan-verifier", "claude-security:patch-generator", "claude-security:patch-verifier", "claude-security:explore)"]
---

<!-- generated by tools/plugins_to_skills.py -->

> Converted from [`plugins/claude-security/agents/claude-security.md`](https://github.com/anthropics/claude-plugins-official/blob/main/plugins/claude-security/agents/claude-security.md)
> in the `claude-security` plugin of `anthropics/claude-plugins-official` (Apache-2.0).
>
> **This was a subagent definition.** The body is written as a system prompt — follow it as your operating instructions for this task. Upstream ran it on `model: opus`.
> Delegate it to a subagent with the Agent tool when the work is large enough to be worth isolating.
>
> `${CLAUDE_PLUGIN_ROOT}` below resolves to `.claude/plugin-skills/vendor/claude-security/` in this repository (except where the text is *teaching* you to write plugins — there it is literal).
