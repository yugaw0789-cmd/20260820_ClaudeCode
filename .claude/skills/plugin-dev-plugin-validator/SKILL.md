...
description: "Use this agent when the user asks to \"validate my plugin\", \"check plugin structure\", \"verify plugin is correct\", \"validate plugin.json\", \"check plugin files\", or mentions plugin validation. Also trigger proactively after user creates or modifies plugin components. See \"When to invoke\" below for worked scenarios."
allowed-tools: ["Read", "Grep", "Glob", "Bash"]
---

<!-- generated by tools/plugins_to_skills.py -->

> Converted from [`plugins/plugin-dev/agents/plugin-validator.md`](https://github.com/anthropics/claude-plugins-official/blob/main/plugins/plugin-dev/agents/plugin-validator.md)
> in the `plugin-dev` plugin of `anthropics/claude-plugins-official` (Apache-2.0).
>
> **This was a subagent definition.** The body is written as a system prompt — follow it as your operating instructions for this task. Upstream ran it on `model: inherit`.
> Delegate it to a subagent with the Agent tool when the work is large enough to be worth isolating.
>
> `${CLAUDE_PLUGIN_ROOT}` below resolves to `.claude/plugin-skills/vendor/plugin-dev/` in this repository (except where the text is *teaching* you to write plugins — there it is literal).
